...
def lose_window(window, player_score, dealer_score):
    ...
    if not window.isClosed():
        while True:
                ...
                if new_game_btn.inside(click2):
                    window.close()
                    game = BlackjackGame()
                    game.start_game()
                    game.draw_cards(window)
                    
                if main_menu_btn.inside(click2):
                    quit_text=Text(Point(600,600), "Quitting the game...")
                    quit_text.setSize(20)
                    quit_text.draw(window)
                    time.sleep(1)
...
def tie_window(window, player_score, dealer_score):
    ...
    if not window.isClosed():
        while True:
            ...
            if new_game_btn.inside(click3):
                window.close()
                game  = BlackjackGame()

                game.start_game()
                game.draw_cards(window)
            if main_menu_btn.inside(click3):
                quit_text=Text(Point(600,600), "Quitting the game...")
                quit_text.setSize(20)
                quit_text.draw(window)
                time.sleep(1)
...
class Card:
    ...
    def get_card_image(self)->str:
        """Return the image of the card based on it's suit and value"""
        if self.value == 1:
            value = "A"
        elif self.value == 11:
            value = "J"
        ...
        return f"{value}_{self.suit[0]}.png"
        
    def show_card(self, window, x, y):
        """show the card image on the window at the x,y position"""
        card_image = Image(Point(x,y), self.get_card_image())
...
class BlackjackGame:
    ...
    def play(self):
        ...
        self.btn_new_game.bind_click(self.win, self.start_game)
